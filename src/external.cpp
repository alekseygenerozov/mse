...
{
    /* Deprecated since June 2021 -- now uses apply_external_perturbation_assuming_integrated_orbits_single_perturber() */
...
{
    /* Deprecated since June 2021 -- now uses apply_external_perturbation_assuming_integrated_orbits_binary_pair_single_perturber() */
...
int apply_external_perturbation_assuming_integrated_orbits_single_perturber(ParticlesMap *particlesMap, double M_per, double e_per, double Q_per, double e_vec_unit_per[3], double h_vec_unit_per[3])
{
    ...
    for (it_p = particlesMap->begin(); it_p != particlesMap->end(); it_p++)
    {
        ...
        if (p->is_binary == true)
        {
            apply_external_perturbation_assuming_integrated_orbits_binary_pair_single_perturber(particlesMap,p->index,M_per,e_per,Q_per,e_vec_unit_per, h_vec_unit_per);
        }
    }
...
void apply_external_perturbation_assuming_integrated_orbits_binary_pair_single_perturber(ParticlesMap *particlesMap, int binary_index, double M_per, double e_f, double q_f, double e_f_hat_vec[3], double j_f_hat_vec[3])
{
    ...
    double abs_a_f = q_f/(e_f-1.0);
    double total_internal_system_mass = flybys_internal_mass; /* make sure this is up to date */
    double n_f = sqrt(CONST_G*(total_internal_system_mass + M_per)/(abs_a_f*abs_a_f*abs_a_f));
